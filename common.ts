...
import { readableStreamToString, SageError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SageError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SageError(e, e.response?.data)
                }
                if (innerError instanceof SageError) {
                    // Got "SageError" from the above try block
                    throw innerError;
                }
